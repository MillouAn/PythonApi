...
class TestUserAuth:
    def test_auth_user(self):
        ...
        assert "user_id" in response2.json(), "There in no user id in the second response"
        user_id_from_check_method = response2.json()["user_id"]

        assert user_id_from_auth_method == user_id_from_check_method, "User id from auth method not equal user id from check method"
    ...
    def test_negative_auth_check(self, condition):
        ...
        assert "user_id" in response1.json(), "There is no user id in second response"
